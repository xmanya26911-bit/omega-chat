...
export function PaymentDialog({ tier, open, onClose }: PaymentDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const accessToken = (await import("@/lib/access-token")).getAccessToken();
            ...
            {step === "submitted" ? (
              <div className="flex flex-col items-center py-6 text-center">
                ...
                <p className="mt-1 text-xs text-[var(--omega-fg-dim)]">
                  We've received your payment request. The admin will verify and activate <strong>{price.label}</strong> within 24h.
                </p>
              ...
              <div className="flex flex-col items-center py-6 text-center">
                ...
                <p className="mt-1 text-xs text-[var(--omega-fg-dim)]">Something went wrong. Please try again.</p>
                <button
              ...
              <div className="py-2">
                <h3 className="text-center text-lg font-semibold text-[var(--omega-fg)]">
                  Enter UPI Reference
                </h3>
                ...
                <div className="mt-3">
                  <label className="block text-xs font-medium text-[var(--omega-fg-dim)] mb-1">
                ...
                <button
                  ...
                  className={cn(
                    "mt-4 w-full rounded-xl px-4 py-3 text-xs font-semibold transition-all",
                    "disabled:opacity-40 disabled:cursor-not-allowed",
                    "bg-[var(--omega-emerald)] text-black hover:brightness-110"
                  )}
                >
                  {submitting ? (
              ...
              /* step === "show" — QR code display */
              <div className="flex flex-col items-center">
                <h3 className="text-lg font-semibold text-[var(--omega-fg)]">
                  Subscribe to {price.label}
                </h3>
                <p className="mt-1 text-xs text-[var(--omega-fg-dim)]">
                ...
                <div className="mt-2 flex gap-4 text-[10px] text-[var(--omega-muted)]">
                  <span>Amount: <strong className="text-[var(--omega-fg)]">₹{price.inr}</strong></span>
                  <span>{price.usd === 10 ? "Monthly" : "Lifetime"}</span>
                </div>
